...
运行：cd element-lawsuit-generator && python3 scripts/test_labor_dispute.py
"""
...
def main():
    ...
    engine.insert_bookmarks(1, 0, [
        ...
        ("T1_05_被告法定代表人职务", 1, 0),
        ("T1_06_被告联系电话", 1, 0),
        ("T1_07_被告统一社会信用代码", 1, 0),
    ])
    
    # 表格1：诉讼请求明细
    engine.insert_bookmarks(1, 3, [
        ("T1_08_工资支付明细", 1, 0),
    ])
    engine.insert_bookmarks(1, 4, [
        ("T1_09_双倍工资明细", 1, 0),
    ])
    engine.insert_bookmarks(1, 5, [
        ("T1_10_加班费明细", 1, 0),
    ])
    engine.insert_bookmarks(1, 6, [
        ("T1_11_未休年休假工资明细", 1, 0),
    ])
    engine.insert_bookmarks(1, 7, [
        ("T1_12_社保损失明细", 1, 0),
    ])
    engine.insert_bookmarks(1, 8, [
        ("T1_13_经济补偿明细", 1, 0),
    ])
    engine.insert_bookmarks(1, 9, [
        ("T1_14_赔偿金明细", 1, 0),
    ])
    engine.insert_bookmarks(1, 11, [
        ("T1_15_其他诉讼请求", 1, 0),
    ])
    engine.insert_bookmarks(1, 12, [
        ("T1_16_标的总额", 1, 0),
    ])
    
    # 表格2：事实与理由
    engine.insert_bookmarks(2, 4, [
        ("T2_04_劳动合同签订情况", 1, 0),
    ])
    engine.insert_bookmarks(2, 5, [
        ("T2_05_劳动合同履行情况", 1, 0),
    ])
    engine.insert_bookmarks(2, 6, [
        ("T2_06_解除终止劳动关系情况", 1, 0),
    ])
    engine.insert_bookmarks(2, 7, [
        ("T2_07_工伤情况", 1, 0),
    ])
    engine.insert_bookmarks(2, 8, [
        ("T2_08_劳动仲裁情况", 1, 0),
    ])
    engine.insert_bookmarks(2, 9, [
        ("T2_09_其他相关情况", 1, 0),
    ])
    engine.insert_bookmarks(2, 10, [
        ("T2_10_诉请依据", 1, 0),
    ])
    
    # --- Step 6: 填充书签内容 ---
    print("\n[Step 6] 填充书签内容...")
    filled_count = engine.fill_all(fill_map)
    print(f"  已填充 {filled_count} 个书签")
    
    # --- Step 7: 填充勾选框 ---
    print("\n[Step 7] 填充勾选框...")
    processed, replaced = engine.fill_checkboxes_squares(checkbox_map)
    print(f"  已处理 {processed} 个勾选框，实际替换 {replaced} 个■")
