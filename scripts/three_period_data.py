...
来源：公安部 GA/T 1193-2014《人身损害误工期、护理期、营养期评定规范》
用途：根据损伤描述自动推导误工期/护理期/营养期

数据结构：
- THREE_PERIOD_DATA: 损伤类型 → (误工期范围, 护理期范围, 营养期范围)
- INJURY_KEYWORD_MAP: 用户描述关键词 → 数据key映射
- lookup_three_period(): 单损伤查询
- lookup_three_period_multi(): 多损伤查询
"""

# ============================================================
# 三期评定数据
# 键名格式：解剖部位_损伤类型_治疗方式
# 值：(误工期范围, 护理期范围, 营养期范围)，单位：天
# ============================================================
THREE_PERIOD_DATA = {
    # ==================== 4.头部损伤 ====================
...
# 用于将自然语言损伤描述（如"右胫腓骨粉碎性骨折"）映射到标准数据
# ============================================================
INJURY_KEYWORD_MAP = {
    # 头部
    "头皮血肿": "头皮血肿_皮下",
...
def lookup_three_period_multi(injury_description: str) -> list:
    ...
    匹配策略：
    1. 清洗修饰词（右侧/左侧/粉碎性/开放性等）
    2. 按关键词匹配所有可能的损伤类型
    3. 按关键词长度降序排列（更具体的优先）

    参数:
        injury_description: 损伤描述文本
